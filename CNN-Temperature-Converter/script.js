...
function loadPreferences() {
    let inputUnit = localStorage.getItem('inputUnit');
    let outputUnit = localStorage.getItem('outputUnit');
    let decimalPlaces = localStorage.getItem('decimalPlaces');
    let theme = localStorage.getItem('theme');

    if (inputUnit) document.getElementById('inputUnit').value = inputUnit;
    if (outputUnit) document.getElementById('outputUnit').value = outputUnit;
    if (decimalPlaces) {
        document.getElementById('decimalPlaces').value = decimalPlaces;
    } else {
        // Set default decimal places to 0 if not set
...
function savePreferences() {
    localStorage.setItem('inputUnit', document.getElementById('inputUnit').value);
    localStorage.setItem('outputUnit', document.getElementById('outputUnit').value);
...
document.getElementById('outputUnit').addEventListener('change', () => {
    savePreferences();
    convertTemperature('input');
});
...
window.onload = function() {
    loadPreferences();
    displayQuizQuestion(); // Start the quiz
    setDailyChallenge();
};
